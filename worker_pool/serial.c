...
typedef struct {
	...
	int* 	total_in;		
	int* 	total_out;
	int* 	shared_index;	// index of the next file to be compressed (all workers read and write this)
	...
	pthread_mutex_t* index_mutex;		// mutex to lock the critical section of changing the shared_index
} worker_args;

/// @brief A worker function that compresses the next available file in the files array and writes to the finished_files array.
/// @param thread_args must unpack to a worker_args struct
/// @return 
void* compress_worker(void* thread_args){
	...
	int* 	shared_index 	= args->shared_index;
	char* 	directory_name 	= args->directory_name;
	char** 	files 			= args->files;
	compressed_file* finished_files = args->finished_files;
	pthread_mutex_t* index_mutex 	= args->index_mutex;

	while(1){
		// get the index of the file to be compressed
		pthread_mutex_lock(index_mutex);   // critical section
		int index = *shared_index;
		*shared_index = *shared_index + 1;
		pthread_mutex_unlock(index_mutex); // \critical section
...
int compress_directory(char *directory_name) {
	...
	for(int i = 0; i < NUM_THREADS; i++){
		...
		thread_args[i].total_in 		= &total_in;
		thread_args[i].total_out 		= &total_out;
		thread_args[i].shared_index 	= &shared_index;
		thread_args[i].directory_name 	= directory_name;
		thread_args[i].files 			= files;
		thread_args[i].finished_files	= finished_files;
		thread_args[i].index_mutex 		= &index_mutex;

		// spawn thread
		pthread_create(&workers[i], NULL, compress_worker, &thread_args[i]);
	}
